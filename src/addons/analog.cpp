...
bool AnalogInput::available() {
    ...
    analogAdcRightPinX = Storage::getInstance().getAddonOptions().analogAdcRightPinX;
	analogAdcRightPinY = Storage::getInstance().getAddonOptions().analogAdcRightPinY;

    return options.AnalogInputEnabled &&
            analogAdcPinX != (uint8_t)-1 && 
            analogAdcPinY != (uint8_t)-1 &&
            analogAdcRightPinX != (uint8_t)-1 && 
            analogAdcRightPinY != (uint8_t)-1;
}
...
void AnalogInput::setup() {
    ...
    adc_gpio_init(analogAdcRightPinX);
    adc_gpio_init(analogAdcRightPinY);
}
...
void AnalogInput::process()
{
    ...
    adc_select_input(2); // ANALOG-X
    float adc_right_x = ((float)adc_read())/ADC_MAX;
    adc_select_input(3); // ANALOG-Y
    float adc_right_y = ((float)adc_read())/ADC_MAX;
    if ( abs(adc_right_x - ANALOG_CENTER) < ANALOG_DEADZONE ) // deadzones
        adc_right_x = ANALOG_CENTER;
    if ( abs(adc_right_y - ANALOG_CENTER) < ANALOG_DEADZONE ) // deadzones
        adc_right_y = ANALOG_CENTER;

    // Convert to 16-bit value
    gamepad->state.lx = (uint16_t)(65535.0f*adc_x);
    gamepad->state.ly = (uint16_t)(65535.0f*adc_y);

    gamepad->state.rx = (uint16_t)(65535.0f*adc_right_x);
    gamepad->state.ry = (uint16_t)(65535.0f*adc_right_y);
}
